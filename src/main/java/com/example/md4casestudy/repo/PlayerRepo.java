...
public interface PlayerRepo extends JpaRepository<Player,Long> {
    ...
    @Modifying
    @Query(value = "select p1.name as player, p1.avatar as avatar, p1.dob as DoB,n.name as nationality,p.name as performance,p2.name as position,ps.name as status,p1.base_salary as base_salary from player p1 join nationality n on n.id = p1.nationality_id join performance p on p.id = p1.performance_id join position p2 on p1.position_id = p2.id join player_status ps on p1.status_id = ps.id where p1.name like ?1",nativeQuery = true)
    Iterable<SearchPlayer> findByName(String name);
    @Modifying
    @Query(value = "select p1.name as player, p1.avatar as avatar, p1.dob as DoB,n.name as nationality,p.name as performance,p2.name as position,ps.name as status,p1.base_salary as base_salary from player p1 join nationality n on n.id = p1.nationality_id join performance p on p.id = p1.performance_id join position p2 on p1.position_id = p2.id join player_status ps on p1.status_id = ps.id where p1.base_salary between ?1 and ?2",nativeQuery = true)
